...
const defaultMainDashboardData = (tenantName: string): MainDashboardData => ({
  title: tenantName,
...
const defaultSecondaryStatsData = (pageCount: number): StatItem[] => [
  {
    title: "Weekly Sales",
    value: "$4,587",
    percentage: "+18%",
...
const StatisticsBlock = ({
  ...
  const activeMainDashboard = mainDashboard || defaultMainDashboardData(tenantName);
  const activeSecondaryStats = secondaryStats || defaultSecondaryStatsData(pageCount);
  ...
  return (
    <div className="grid grid-cols-12 gap-6 h-full">
            ...
            <div className="ps-6 py-4 flex flex-col gap-9 justify-between">
              <div>
                <p className="text-lg font-medium text-card-foreground">
                  {activeMainDashboard.title}
                </p>
                <p className="text-xs font-normal text-muted-foreground">
                  {activeMainDashboard.description}
                </p>
              </div>
              <div className="flex items-center gap-6">
                {activeMainDashboard.metrics.map((metric, index) => (
                  <div key={index} className="flex items-center gap-6">
                    ...
                    {index < activeMainDashboard.metrics.length - 1 && (
      ...
      {activeSecondaryStats.map((stat, index) => (
